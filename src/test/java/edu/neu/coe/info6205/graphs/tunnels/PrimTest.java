...
public class PrimTest {
    ...
    public void testPrim() {
        ...
        Edge<String, Route> edge1 = edgeList.get(0);
        Edge<String, Route> edge2 = edgeList.get(1);
        Edge<String, Route> first = edgeList.get(0);
        String startpoint = first.get(); //take "pa" as the starting point
        Iterable<Edge<String, Route>> iterable = new Prim<>(kalimantan,startpoint);
        Iterator<Edge<String, Route>> iterator = iterable.iterator();
        assertTrue(iterator.hasNext());
        //debug only-list out all "steps" to complete the spanning
        /*
        while (iterator.hasNext()) {
        	Edge<String,Route> cur = iterator.next();
        	System.out.println(cur.get()+", "+cur.getOther(cur.get()));
