...
public class UF_HWQUPC implements UF {
    ...
    public int find(int p) {
        ...
        //Check whether the parent is not the root
        while(parent[root] != root) {
            root = parent[root];
        }
        //If the path compression flag is enabled, do path compression
        if(pathCompression) {
    ...
    private void mergeComponents(int i, int j) {
        ...
        if(i!=j){
            if(height[i] >= height[j]){ //If the height of first site is greater than height of second site
                parent[j] = i; //Parent of second site will be the first site
                updateHeight(i, j); //Update the height of both sites
            }
            else{
                parent[i] = j; //Parent of first site will be the second site
                updateHeight(j, i); //Update the height of both sites
            }
        }
    }
    ...
    private void doPathCompression(int i, int root) {
        ...
        while(i != root){ //Loop until "i" is not the root
            int j = parent[i];
            parent[i] = root;
            i = j;
        }
